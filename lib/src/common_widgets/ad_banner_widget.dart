...
class _AdBannerWidgetState extends ConsumerState<AdBannerWidget> {
  ...
  void _evaluate() {
    ...
    bool shouldShow;
    if (mode != AdDisplayMode.banner) {
      shouldShow = false;
      analytics.logAdBannerSkipped(reason: 'Not banner mode');
    } else if (!ctx.isPortrait) {
      shouldShow = false;
      analytics.logAdBannerSkipped(reason: 'Not portrait mode');
    } else if (!ctx.isRevealMode) {
      shouldShow = false;
      analytics.logAdBannerSkipped(reason: 'Not reveal mode');
    } else {
      shouldShow = true;
    }

    if (_lastShouldShow != shouldShow) {
