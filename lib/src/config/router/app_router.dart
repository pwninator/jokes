...
enum TabId { feed, discover, saved, settings, admin }
...
class TabConfig {
  ...
  /// Check if this tab is visible given the current admin status
  bool isVisible({required bool isAdmin}) {
...
List<TabConfig> _visibleTabs(bool isAdmin) {
  return _allTabs.where((t) => t.isVisible(isAdmin: isAdmin)).toList();
}
...
class NavigationState {
  ...
  factory NavigationState.create({
    ...
    final visibleTabs = _visibleTabs(isAdmin);
...
class AppRouter {
  ...
  static String get homepageRoute => AppRoutes.feed;
