...
from common.posable_character_sequence import (
  ...
  SequenceSoundEvent, SequenceTransformEvent)
...
class PosableCharacterGeneratorTest(unittest.TestCase):
  ...
  def test_generate_frames_duration(self):
    ...
    fps = 10
    frames = list(CharacterAnimator(seq).generate_frames(self.char, fps))
  ...
  def test_sound_events(self):
    ...
    seq = PosableCharacterSequence(sequence_sound_events=[
      SequenceSoundEvent(start_time=0.5, end_time=1.0, gcs_uri="uri", volume=1.0)
    ])
  ...
  def test_interpolation(self):
    ...
    seq = PosableCharacterSequence(sequence_left_hand_transform=[
      SequenceTransformEvent(start_time=0.0,
                             end_time=1.0,
                             target_transform=Transform(translate_x=10))
    ])
    fps = 2  # dt = 0.5. Frames: 0.0, 0.5, 1.0.

    gen = CharacterAnimator(seq).generate_frames(self.char, fps)
  ...
  def test_gap_behavior(self):
    ...
    seq = PosableCharacterSequence(sequence_left_hand_transform=[
      SequenceTransformEvent(start_time=0.0,
                             end_time=1.0,
                             target_transform=Transform(translate_x=10)),
      SequenceTransformEvent(start_time=2.0,
                             end_time=3.0,
                             target_transform=Transform(translate_x=20)),
    ])
  ...
  def test_mouth_gap_behavior(self):
    ...
    seq = PosableCharacterSequence(sequence_mouth_state=[
      SequenceMouthEvent(
        start_time=0.0, end_time=1.0, mouth_state=MouthState.OPEN),
      SequenceMouthEvent(
        start_time=2.0, end_time=3.0, mouth_state=MouthState.O),
    ])
