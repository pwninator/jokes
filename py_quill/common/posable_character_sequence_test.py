...
class PosableCharacterSequenceTest(unittest.TestCase):
    def test_serialization(self):
        seq = PosableCharacterSequence(
            ...
            sequence_mouth_state=[
                SequenceMouthEvent(start_time=0.5, end_time=1.5, mouth_state=MouthState.OPEN),
            ],
            ...
            sequence_sound_events=[
                SequenceSoundEvent(start_time=0.0, gcs_uri="gs://bucket/sound.wav", volume=0.8),
            ]
        )
        ...
        self.assertEqual(data["sequence_left_hand_transform"][0]["target_transform"]["translate_x"], 10)
        ...
        self.assertEqual(seq2.sequence_left_hand_transform[0].target_transform.translate_x, 10)
    ...
    def test_validation_overlap(self):
        seq = PosableCharacterSequence(
            sequence_left_eye_open=[
                SequenceBooleanEvent(start_time=0.0, end_time=1.0, value=True),
                SequenceBooleanEvent(start_time=0.5, end_time=1.5, value=False), # Overlap
            ]
        )
    ...
    def test_sound_validation(self):
        ...
        seq = PosableCharacterSequence(
            sequence_sound_events=[
                SequenceSoundEvent(start_time=0.0, gcs_uri="uri1"),
                SequenceSoundEvent(start_time=0.5, gcs_uri="uri2"),
            ]
        )
        seq.validate()
