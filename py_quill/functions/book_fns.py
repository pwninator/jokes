...
_BASIC_VOICE_MODELS = (
    gen_audio.Voice.EN_US_STANDARD_FEMALE_1,
)
...
_ADVANCED_VOICE_MODELS = (
    gen_audio.Voice.EN_US_CHIRP3_HD_FEMALE_LEDA,
)
...
def _critical_error(book_ref: firestore_fn.DocumentReference,
  ...
  _record_timing_event(book_data, book_ref, 'Critical Error', {'error': error_msg})
...
def _add_minor_error(book_ref: firestore_fn.DocumentReference,
  ...
  _record_timing_event(book_data, book_ref, 'Minor Error', {'error': error_msg})
...
def on_book_created(
  ...
  try:
    ...
    if book_data and book_data.get('title'):
      ...
      chapters = list(book_snapshot.reference.collection(
          'chapters').limit(1).stream())
      if chapters:
        log_info(
            f"Book {book_snapshot.id} already populated with title and chapters. "
            "Skipping generation.", book_id=book_id, user_id=user_id)
    ...
    while attempt <= max_attempts:
      log_info(
          f"Attempt {attempt}/{max_attempts} to generate book {book_snapshot.id}", book_id=book_id, user_id=user_id)
      ...
      try:
        ...
        log_warning(f"Book generation attempt {attempt} failed with error: {e.message}",
                    book_id=book_snapshot.id,
                    user_id=book_snapshot.get('owner_user_id'))
        attempt += 1
        ...
        if attempt > max_attempts:
          log_error(f"All {max_attempts} attempts to generate book {book_snapshot.id} failed",
                    book_id=book_id, user_id=user_id)
          book_ref.update({
              'status': 'error',
              'final_error': f"Failed after {max_attempts} attempts: {e.message}",
              'last_modification_time': SERVER_TIMESTAMP,
          })
          raise
...
def populate_book(req: https_fn.Request) -> https_fn.Response:
  ...
  if not book_id:
    return https_fn.Response(
        json.dumps({"error": "Missing book_id parameter"}),
        status=400,
  ...
  if not book_doc.exists:
    return https_fn.Response(
        json.dumps({"error": f"Book {book_id} not found"}),
        status=404,
        mimetype='application/json'
    )
...
def populate_page(req: https_fn.Request) -> https_fn.Response:
  ...
  if not all([book_id, chapter_id, page_id]):
    return https_fn.Response(
        json.dumps(
            {"error": "Missing required parameters. Need book_id, chapter_id, and page_id"}),
        status=400,
        mimetype='application/json'
    )
  ...
  if not page_doc.exists:
    return https_fn.Response(
        json.dumps({"error": f"Page {page_id} not found"}),
        status=404,
        mimetype='application/json'
    )
...
def on_page_created(event: firestore_fn.Event[firestore_fn.DocumentSnapshot | None]) -> None:
  ...
  if not page_snapshot:
    log_warning("No snapshot in on_page_created event", book_id=None, user_id=None)
    return
...
def _start_page_image_generation(
...
) -> tuple[str | None, Callable[[dict[str, bytes] | None], models.Image] | None]:
  ...
  if not illustration_data.description:
    log_error(f"Skipping image generation for page {page_number} due to missing description.",
              book_id=book_ref.id,
  ...
  if not owner_user_id:
    raise ValueError(f"No owner user ID for page {page_number} in book {book_ref.id}")
  ...
  try:
    ...
    _record_timing_event(book_data, book_ref, f"Page {page_number} - Image Kickoff")
...
def _generate_page_audio(
  ...
  if not page_data:
    _add_minor_error(
        book_ref, f"Cannot generate audio for page {page_doc_snapshot.id}: No page data found.")
    return None, audio_metadata

  if not book_data:
    _add_minor_error(book_ref,
                     f"Cannot generate audio for page {page_doc_snapshot.id}: "
  ...
  if audio_voice_str:
    try:
      ...
      _add_minor_error(book_ref, f"Invalid audio_voice value: {audio_voice_str}.")
  ...
  if not page_text:
    _add_minor_error(book_ref, f"Cannot generate audio for page {page_number}: No text found.")
    return None, audio_metadata
  ...
  try:
    ...
    _record_timing_event(book_data, book_ref, f"Page {page_number} - Audio Generated")

  except gen_audio.GenAudioError as e:
    _add_minor_error(book_ref, f"Failed to generate audio for page {page_number}: {e}")
...
def _finish_page_image_generation(
  ...
  try:
    ...
    _record_timing_event(book_data, book_ref, f"Page {page_number} - Image Processing Start")
    ...
    _record_timing_event(book_data, book_ref, f"Page {page_number} - Image Processed")
...
def _populate_page(page_doc_snapshot: DocumentSnapshot) -> models.GenerationMetadata:
  ...
  page_number = page_data.get("page_number", "?")
  _record_timing_event(book_data, book_ref, f"Page {page_number} - Populate Start")
  ...
  if book_data.get('characters'):
    for name, char_dict in book_data.get('characters', {}).items():
      character_descriptions[name] = models.StoryCharacterData.from_dict(char_dict)
  ...
  page_image_key, page_image_callback = _start_page_image_generation(
      book_data, book_ref, page_number, illustration_data, character_descriptions
  )
  ...
  audio_gcs_uri, audio_meta = _generate_page_audio(book_data, book_ref, page_doc_snapshot)
  ...
  if audio_gcs_uri:
    page_doc_snapshot.reference.update({
        ...
        'generation_metadata': total_generation_metadata.as_dict,
        'last_modification_time': SERVER_TIMESTAMP,
    })

  # 3. Finish Image Generation (Blocking)
  if page_image_callback:
    image_finish_meta = _finish_page_image_generation(
        book_data, book_ref, page_number, page_image_callback, page_doc_snapshot
    )
...
def _populate_book(
  ...
  try:
    ...
    for chapter in chapters:
      ...
      if delete_existing_pages:
        ...
        if list(pages_collection.limit(1).stream()):
          logger.info(f"Book {book_ref.id} already has pages. Skipping generation.")
          return models.GenerationMetadata()

    # Get characters
    main_characters, side_characters = _get_characters(
        main_character_keys, side_character_keys)
    ...
    try:
      recent_stories = firestore.get_recent_stories(owner_user_id, limit=_NUM_RECENT_STORIES)
      recent_story_titles = [
    ...
    try:
      ...
      joke_generator = joke_prompts.generate_jokes(
          joke_prompt, extra_log_data=_get_extra_log_data(book_ref.id, owner_user_id))
      jokes_metadata = get_and_store_jokes(book_data, book_ref, joke_generator, num_jokes=1)
    ...
    try:
      ...
      if not (plot and learning_topic and learning_concepts and wiki_url):
        ...
        _record_timing_event(book_data, book_ref, 'Choose/Fetch Reference Material')
      else:
        reference_material_full_text = wikipedia.get_text(learning_topic)
        _record_timing_event(book_data, book_ref, 'Fetch Wikipedia')
    except Exception as e:  # pylint: disable=broad-exception-caught
      raise _critical_error(book_ref, f"Failed to get reference material: {str(e)}") from e
    ...
    try:
      ...
      raise _critical_error(book_ref, f"Failed to initialize story generation: {str(e)}") from e
    ...
    try:
      ...
      _add_minor_error(book_ref, f"Failed to generate remaining jokes: {str(e)}")
    ...
    try:
      for incremental_story_data, incremental_metadata in story_generator:
        ...
        if cover_image_callback is not None:
          try:
            ...
            _add_minor_error(book_ref, f"Failed to process cover image: {str(e)}")
        ...
        for k in updated_keys:
          match k:
            ...
            case 'plot_brainstorm':
              _add_debug_data(book_ref, 'plot_brainstorm', story_data.plot_brainstorm)
            case 'outline':
              ...
              for page in incremental_story_data.pages:
                if not page.is_complete:
                  raise _critical_error(
                      book_ref, f"Page {page.page_number} is not complete: {page.as_dict}")

                if page.page_number != max_page_number_seen + 1:
                  raise _critical_error(
                      book_ref,
                      f"Expected page {max_page_number_seen + 1}, "
                ...
                try:
                  ...
                  raise _critical_error(
                      book_ref, f"Failed to add page {page.page_number}: {str(e)}") from e

                max_page_number_seen = page.page_number
                _record_timing_event(book_data, book_ref, f'Page {page.page_number} - Start')

            case 'characters':
              update_dict['characters'] = {
                  name: char.as_dict for name, char in story_data.characters.items()
              }
            ...
            case 'learning_concepts':
              update_dict.update({
                  ...
                  'learning_concepts': {
                      name: concept.as_dict for name, concept
                      in story_data.learning_concepts.items()},
              })
            ...
            case 'cover_illustration':
              if not (story_characters := story_data.characters):
                _add_minor_error(
                    book_ref, "No character descriptions found for cover illustration")
                story_characters = {}

              try:
                # Generate detailed cover illustration description
                cover_image_prompt, cover_image_backup_prompt = _get_illustration_prompt(
                    story_data.cover_illustration, story_characters, book_ref.id, owner_user_id)

                # Kick off generation for the cover image
                _record_timing_event(book_data, book_ref, 'Cover Image - Start')
                ...
                # TODO: Add a retry mechanism for cover image generation
                _add_minor_error(book_ref, f"Failed to generate cover image: {str(e)}")
            case _:
      ...
      raise _critical_error(book_ref, f"Error during story generation: {str(e)}") from e
...
def _get_and_validate_book_params(
  ...
  llm_model, audio_voice = _get_models_for_story_mode(
      story_mode, llm_model_override, audio_voice_override)
  ...
  reading_level_value = book_data.get('reading_level', ReadingLevel.THIRD.value)
...
def _get_characters(main_character_keys: list[str], side_character_keys: list[str]
  ...
  main_characters = [characters_by_key[key]
                     for key in main_character_keys if key in characters_by_key]
  side_characters = [characters_by_key[key]
                     for key in side_character_keys if key in characters_by_key]
...
def _get_reference_material(
  ...
  past_topics = []
  if recent_stories:
    past_topics = [story.get('learning_topic', None)
  ...
  generation_metadata = models.GenerationMetadata()
  (
      plot, wikipedia_title, plot_response_dict, plot_metadata
  ) = reference_material.generate_plot_and_wiki_title(
      story_prompt, characters, past_topics, reading_level,
      extra_log_data=_get_extra_log_data(book_ref.id, book_data=book_data),
  )
  ...
  try:
    ...
    raise _critical_error(
        book_ref, f"Error searching Wikipedia for '{wikipedia_title}': {e}") from e

  raise ValueError("Could not find relevant Wikipedia content")


def _get_reference_images(book_ref: firestore_fn.DocumentReference) -> dict[str, bytes]:
  ...
  char_name_by_portrait_key = {
      ...
      if character.portrait_image_key
  }
...
def _get_illustration_prompt(
  ...
  if illustration_data.characters and character_descriptions:
    ...
    main_prompt = [illustration_descriptions.refine_illustration_description(
        illustration_data.description,
        {name: char.visual for name, char in character_descriptions.items()},
...
def _add_debug_data(book_ref: firestore_fn.DocumentReference, label: str, data: Any) -> None:
...
def _record_timing_event(
  ...
  if not creation_time:
    raise ValueError(f"Book document {book_ref.id} does not have a creation time")
  ...
  book_ref.collection('timing_events').document(timing_event_id).set(timing_event)
  ...
  logger.info(f"Timing event: {event_type}", extra={"json_fields": json_fields})
...
def get_and_store_jokes(
  ...
  try:
    ...
    _record_timing_event(book_data, book_ref, 'Jokes', {'count': total_jokes_stored})
...
def _store_jokes_batch(
    jokes_collection: CollectionReference,
    jokes_batch: List[str]
) -> None:
